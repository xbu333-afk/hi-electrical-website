  ...
  async function signInWithGoogle() {
    ...
    try {
      ...
      if (error) {
        ...
        setLocalError("Google OAuth לא מוגדר ב-Supabase — כנס ל-Authentication → Providers → Google");
        setLoading(false);
      }
    } catch (e) {
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-10 w-full max-w-sm text-center space-y-6">
        ...
        {displayError && (
          <div className="bg-red-950 border border-red-700 text-red-300 text-xs rounded-lg px-4 py-3 text-right break-all">
            <strong>שגיאה:</strong><br />{displayError}
          </div>
        )}
        ...
        >
          {loading ? <span className="animate-spin">⏳</span> : <GoogleIcon />}
